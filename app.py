...
def infer_quarter_label(workbook_name: str | None, snapshot_label: str | None) -> str:
    name = (workbook_name or "").strip() or (snapshot_label or "").strip()
    patterns = [r"(\b[1-4]Q\d{2}\b)", r"(\b[1-4]Q\d{4}\b)"]
    ...
    return f"{quarter}Q{str(today.year)[-2:]}"
...
def clear_uploaded_workbook() -> None:
    for key in ["workbook_bytes", "workbook_name", "file_hash", "editor_df", "selected_row"]:
        st.session_state.pop(key, None)
...
def render_sharepoint_browser(config: dict[str, str]) -> tuple[bytes | None, str | None]:
    ...
    account_name = (token_info.get("id_token_claims") or {}).get("name") or (
        token_info.get("id_token_claims") or {}
    ...
    selected_drive_name = st.selectbox(
        ...
        index=list(drive_options.keys()).index(
            st.session_state.get("sharepoint_selected_drive_name", default_drive_name)
        ),
    ...
    if folder_names:
        next_folder = st.selectbox(
            "Open folder",
            [""] + folder_names,
            key=f"folder_select_{selected_drive_id}_{current_folder_id or 'root'}",
        )
        if next_folder:
    ...
    file_labels = [f"{item['name']} · modified {item.get('lastModifiedDateTime', '')[:10]}" for item in excel_files]
...
def get_active_workbook() -> tuple[bytes | None, str | None]:
    ...
    with st.expander("Change workbook", expanded=False):
        ...
        replacement = st.file_uploader("Replace current workbook", type=["xlsx"], key="replace_workbook_upload")
...
def apply_filters(display_df: pd.DataFrame) -> tuple[pd.DataFrame, date, str, str, str]:
    with st.sidebar:
        ...
        workspace_mode = st.radio("Workspace", ["Review workspace", "Overview", "Bulk update"], index=0)
        ...
        asset_managers = sorted(
            [x for x in display_df["Asset Manager"].dropna().astype(str).unique().tolist() if x]
        )
        st.markdown("**Asset Manager review**")
        am_scope = st.selectbox(
            "Show deals for",
            ["All asset managers"] + asset_managers,
            index=0,
            help="Choose one AM here to only review that AM's loans.",
            key="am_scope_select",
        )
        review_order = st.radio(
            ...
            index=0,
            help="Choose whether to keep the uploaded workbook order or review grouped by Asset Manager.",
            key="review_order_radio",
        )
        ...
        sort_mode = st.selectbox(
            ...
            help="This is the secondary sort. Queue order above is the main presentation choice.",
    ...
    if review_order == "Workbook file order":
...
def render_queue_navigation(sorted_df: pd.DataFrame) -> None:
    ...
    with nav3:
        selected_row = st.selectbox(
            "Deal queue",
            option_rows,
            format_func=lambda row_num: format_deal_label(
                sorted_df[sorted_df["_excel_row"] == row_num].iloc[0]
            ),
...
def editable_field_input(field: str, current_value: str, picklists: dict[str, list[str]]) -> Any:
    if field in MANUAL_ENTRY_PICKLIST_FIELDS and field in picklists:
        ...
        if selected_option == custom_option:
            return st.text_input(field, current_value, key=f"{field}_custom_{abs(hash((field, current_value, 'custom')))}")
...
def render_snapshot_grid(selected: pd.Series) -> None:
    ...
    for section_title, rows in sections:
        ...
        for label, value in rows:
            html_parts.append(f"<div class='snapshot-row'><div class='snapshot-label'>{label}</div><div class='snapshot-value'>{value}</div></div>")
        html_parts.append("</div>")
    html_parts.append("</div>")
    st.markdown("".join(html_parts), unsafe_allow_html=True)


def render_review_form(selected: pd.Series, picklists: dict[str, list[str]], properties_df: pd.DataFrame, comment_date: date) -> dict[str, Any] | None:
    ...
    with st.form(f"detail_form_{int(selected['_excel_row'])}"):
        ...
        with left:
            ...
            updates["Needs Discussion"] = st.checkbox("Needs discussion", value=bool(selected.get("Needs Discussion", False)))
        with middle:
        ...
        with right:
            ...
            with st.expander("Full prior comment history", expanded=False):
                st.text_area("Comment history", editable_text(selected.get("Comment History")), disabled=True, height=220)
            ...
            st.caption(
                f"Addt'l NPL Comment will save as: {format_comment_entry_date(comment_date)} - {built_comment}"
                if built_comment else "No current-week comment entered yet."
            )
            st.text_area(
        ...
        if submitted or submitted_next or submitted_next_blank:
            updates["_advance_mode"] = "next" if submitted_next else ("next_blank" if submitted_next_blank else "stay")
            return updates
    return None
...
def render_bulk_edit(filtered_df: pd.DataFrame, picklists: dict[str, list[str]]) -> pd.DataFrame | None:
    ...
    column_config: dict[str, Any] = {"Needs Discussion": st.column_config.CheckboxColumn("Needs Discussion")}
    ...
    if workspace_mode == "Overview":
        ...
        if updates:
            ...
            if advance_mode == "next" and current_idx < len(option_rows) - 1:
                ...
                blanks = queue_after_save[queue_after_save["This Week Comment"].fillna("").astype(str).str.strip().eq("")]["_excel_row"].tolist()
